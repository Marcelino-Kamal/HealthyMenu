    ...
    <div className="fixed inset-0  bg-opacity-40 backdrop-blur-xs flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md">
        ...
        <p className="text-sm text-gray-600 mb-2">
          Health Score: {meal.healthScore}/10
        </p>
        <p className="text-sm text-gray-600 mb-2">
          Difficulty: {meal.difficulty}
        </p>
        <p className="text-sm text-gray-600 mb-2">Time: {meal.duration}</p>
        <p className="text-sm text-gray-600 mb-2">{meal.gradients}</p>
        <p className="text-sm text-gray-600 mb-2">{meal.steps}</p>
        <button
        ...
        <button
          onClick={onClose}
          className="mt-4 bg-[#2682C0] text-white py-2 px-4 rounded-full"
        >
          Close
        </button>
      </div>
